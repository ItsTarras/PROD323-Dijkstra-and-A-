namespace twe36
{
    ...
    public static class PathAlgorithm
    {
        ...
        public static int maxSlopeAngle = 50;
        ...
        // checks if the current node and an adjacent noed are above a certain angle, and if so, don't return it.
        public static float CalculateSlope(Node currentNode, Node adjacentNode)
        {
            // Calculate the direction vector from currentNode to adjacentNode
            float deltaX = adjacentNode.nodePosition.X - currentNode.nodePosition.X;
            float deltaY = adjacentNode.nodePosition.Y - currentNode.nodePosition.Y;
            float deltaZ = adjacentNode.nodeHeight - currentNode.nodeHeight;

            // Calculate the slope angle using the direction vector
            float slopeAngle = Mathf.Atan2(deltaZ, Mathf.Sqrt(deltaX * deltaX + deltaY * deltaY)) * Mathf.Rad2Deg;

            // Ensure the slope angle is within the range [0, 360]
            slopeAngle %= 360;

            // Return the absolute value of the slope angle
            return Mathf.Abs(slopeAngle);
        }
